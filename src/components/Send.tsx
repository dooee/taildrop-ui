...
import { sendPaths, type Target, type SendResult } from '../tailscale.js';
import { archiverInstallHint } from '../zip.js';
import { useT } from '../i18n.js';
import type { NameIssue } from '../paths.js';
...
type Stage = 'browse' | 'pickTarget' | 'sending' | 'done';
...
const NAME_REASON_KEY: Record<NameIssue, string> = {
...
export default function Send({ bundleMultiple, onDone }: Props) {
  const t = useT();
  ...
  const startSend = async (target: Target) => {
    setStage('sending');
    const res = await sendPaths(paths, target.name, { bundleMultiple }, (msg) =>
      setLogs((prev) => [...prev, decodeLog(msg)]),
    );
    setResult(res);
    setStage('done');
  };
  ...
  if (stage === 'browse') {
    return (
      <FileBrowser
        mode="files"
        title={t('browser.filesTitle')}
        bundleMultiple={bundleMultiple}
        onSubmit={(selected) => {
          setPaths(selected);
          setStage('pickTarget');
        }}
  ...
  if (stage === 'pickTarget') {
    return (
      <TargetPicker onSelect={startSend} onCancel={() => setStage('browse')} />
    );
  }

  if (stage === 'sending') {
    return (
      <Frame screen={t('send.sendingScreen')}>
        <Box flexDirection="column">
          <Text color="cyan">◇ {t('send.sending')}</Text>
          {logs.map((l, i) => (
            <Text key={i} dimColor>
              {'  '}
              {l}
  ...
  // done · 완료
  return (
    <Frame screen={t('send.doneScreen')} footer={t('common.enterMenu')}>
      <Box flexDirection="column">
        {result?.ok ? (
          <>
            <Text color="green">
          ...
          <>
            <Text color="red">✘ {t('send.fail')}</Text>
            {result?.errorCode === 'archiver-missing' ? (
              <>
                <Text>{t('send.err.archiverMissing')}</Text>
                <Text dimColor>{archiverInstallHint()}</Text>
              </>
            ) : result?.nameProblems ? (
            ...
            ) : result?.errorCode && ERR_KEY[result.errorCode] ? (
              <>
                <Text>{t(ERR_KEY[result.errorCode]!)}</Text>
                {result.error ? <Text dimColor>{result.error}</Text> : null}
              </>
            ) : (
