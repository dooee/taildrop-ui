 ...
 * and the CLI help/usage/errors — so their keys must count as used, or they
 * would read as orphans. cli-core.ts stays out of UI_FILES because it has no
 * hardcoded Hangul to scan for.
 *
 ...
 * 셋업 안내와 CLI 도움말·usage·오류 — 그 키들이 사용된 것으로 집계되지 않으면
 * 고아로 읽힌다. cli-core.ts 는 스캔할 하드코딩 한글이 없어 UI_FILES 에는 넣지 않는다.
 */
