...
> the terminal — file browser, device picker. Command: **`tdui`**.

```
 🚀 Taildrop  tailscale 파일 송수신  › 메뉴
──────────────────────────────────────────────
무엇을 할까요?
...
기능입니다. taildrop-ui는 그 Taildrop을 **실제로 편하게** 쓰게 하려고 만들었습니다 — `tailscale`
...
나중에 업데이트하려면 `npm install -g taildrop-ui@latest` 를 실행하세요.
...
npm install -g .     # 전역 명령 `tdui` 등록
```

> `npm link` 로도 되며 결과(전역 `tdui`)는 완전히 같습니다 — 차이는 연결 방식뿐입니다.
> `npm install -g .` 은 빌드된 **복사본**을 설치하고, `npm link` 는 이 **작업 폴더를 심볼릭 링크**로
> 연결합니다. 그래서 소스를 직접 고칠 계획이면 `npm link` 가 낫습니다: `npm run build` 만 하면
> 재설치 없이 `tdui` 에 바로 반영됩니다.

## 사용법

`tdui` 실행 → 메뉴에서 방향키 이동, Enter 선택.
...
| Linux | `$XDG_CONFIG_HOME/taildrop-ui/config.json` (기본 `~/.config/...`) |
...
taildrop-ui는 **독립적인 비공식** 프런트엔드이며, Tailscale Inc.와 제휴하거나 인증받은 관계가
